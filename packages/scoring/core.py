...
LABELS = dict(
    zip(
        COMPONENTS,
        ["Demand Potential", "Access", "White Space", "Ecosystem", "Cost Efficiency", "Operational Context"],
        strict=True,
    )
)
PROFILE_ROWS = [
    (
        "coffee",
        "Coffee Shop",
        [0.27, 0.18, 0.25, 0.13, 0.12, 0.05],
        ["office", "university", "transport", "coworking", "hotel", "retail"],
    ),
    ("bakery", "Bakery", [0.29, 0.16, 0.24, 0.14, 0.12, 0.05], ["transport", "school", "coffee", "retail"]),
    (
        "restaurant",
        "Restaurant",
        [0.26, 0.17, 0.22, 0.18, 0.12, 0.05],
        ["retail", "hotel", "entertainment", "pub_bar", "office", "transport"],
    ),
    ("gym", "Gym", [0.24, 0.15, 0.26, 0.10, 0.20, 0.05], ["office", "transport", "retail"]),
    (
        "convenience",
        "Convenience Store",
        [0.30, 0.18, 0.25, 0.08, 0.14, 0.05],
        ["transport", "retail", "office", "hotel"],
    ),
    (
        "coworking",
        "Coworking Space",
        [0.25, 0.22, 0.20, 0.17, 0.11, 0.05],
        ["office", "coffee", "transport", "hotel", "retail"],
    ),
]
PROFILES = {
    key: {
        ...
        "label": "PULSE default business profiles",
    }
...
def percentile(values, inverse=False, log_transform=True):
    ...
    if good.sum() <= 1:
        return out
    a = x[good]
    a = np.clip(a, *np.quantile(a, [0.01, 0.99]))
    if log_transform:
    ...
    contributions = [
        {
            ...
            "value": components[k],
            "weight": w[k],
            "contribution": confidence * w[k] * (components[k] - 50),
        }
        for k in COMPONENTS
    ]
    positives = sorted(
        (r for r in contributions if r["contribution"] > 0), key=lambda r: r["contribution"], reverse=True
    )
