...
def fit_white_space(x, observed, groups, coordinates):
    ...
    if np.count_nonzero(y) < 40 or y.sum() < 100:
        ...
        return expected, np.full(len(y), .4), {
            'family': 'shrunken_exposure_ratio', 'coefficients': [], 'observations': len(y),
            'positive_cells': int(np.count_nonzero(y)), 'validation': 'Sparse category fallback; predictive fit not claimed',
            'cv_mae': None, 'overdispersion': None,
            'parameters': {'prior_count': 2, 'prior_exposure': 2}}, (expected-y)/np.sqrt(expected+1)
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')
        poisson = sm.GLM(y, design, family=sm.families.Poisson()).fit(maxiter=100)
    mu = np.maximum(poisson.predict(design), .001)
    dispersion = float(np.sum((y - mu)**2 / mu) / max(len(y) - design.shape[1], 1))
    alpha = max(float(np.sum((y - mu)**2 - mu) / max(np.sum(mu**2), 1e-9)), .01)
    ...
    expected = np.maximum(model.predict(design), .001)
    ...
    for train, test in GroupKFold(n_splits=min(4, len(set(groups)))).split(design, y, groups):
        ...
        try:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                fit = sm.GLM(y[train], design[train], family=family()).fit(maxiter=100)
            pred = np.clip(fit.predict(design[test]), .001, max(y.max()*10, 10))
            oof[test] = pred
            folds.append({'train': len(train), 'test': len(test), 'mae': float(mean_absolute_error(y[test], pred)),
    ...
    baseline_mae = float(mean_absolute_error(y[valid], np.full(valid.sum(), y.mean()))) if valid.any() else None
    skill = max(0, 1 - mae / max(baseline_mae, .001)) if mae is not None else 0
    # A quality index, not a calibrated probability or confidence interval.
    quality = .45 + .30 * skill
    confidence = np.clip(quality / (1 + np.maximum(expected - np.quantile(expected, .95), 0)
                                   / max(np.quantile(expected, .95), 1)), .25, .8)
    variance = expected + (alpha * expected**2 if dispersion > 1.5 else 0)
    residual = (expected - y) / np.sqrt(variance + 1)
    metrics = {'family': family_name, 'coefficients': model.params.tolist(), 'observations': len(y),
               'positive_cells': int(np.count_nonzero(y)), 'overdispersion': dispersion,
               'alpha': alpha if dispersion > 1.5 else None, 'cv_mae': mae, 'cv_baseline_mae': baseline_mae,
               'cv_coverage': float(valid.mean()), 'folds': folds,
               'validation': '4-fold H3 resolution-5 groups with 800m exclusion buffer',
               'prediction_quantiles': np.quantile(expected, [0, .25, .5, .75, .95, 1]).tolist(),
               'parameters': {'means': mean.tolist(), 'scales': scale.tolist(), 'alpha': alpha,
                              'predictors': ['population_density', 'station_proximity', 'complementary_venues',
                                             'high_street_proximity', 'occupancy_cost_pressure']}}
