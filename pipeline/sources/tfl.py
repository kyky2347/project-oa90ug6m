...
def normal_name(name):
    s = re.sub(r'\b(underground|overground|rail|railway|station|dlr|lu|nr|elizabeth line|tram stop)\b', '', name.lower())
...
def parse_flows(workbook):
    ...
    for sheet in ['Station_Entries', 'Station_Exits']:
        ...
        periods = [(i, int(v[:2]) % 24 * 4 + int(v[2:4]) // 15) for i, v in enumerate(header)
...
class TfLSource(Source):
    ...
    def discover(self):
        ...
        while True:
            ...
            entries.extend((e.find('s:Key', NS).text, e.find('s:LastModified', NS).text)
        ...
        for k, modified in files:
            if f'NUMBAT {year}/' in k:
                day = {'TWT': 'weekday', 'SAT': 'saturday', 'SUN': 'sunday'}[re.search(r'NBT\d{2}(TWT|SAT|SUN)', k).group(1)]
                out.append(Asset('https://crowding.data.tfl.gov.uk/' + quote(k), day + '.xlsx',
                                 f'{year}-11-01', {'year': year, 'day': day, 'released_at': modified}))
        out.append(Asset('https://api.tfl.gov.uk/StopPoint/Mode/tube,dlr,overground,elizabeth-line', 'stops.json'))
    ...
    def transform(self, directory, assets):
        ...
        for a in assets:
            ...
            for sid, row in flows.items():
                ...
                if a.extra['day'] == 'weekday' or sid not in stations:
                    stations[sid] = dict(id=sid, name=row['name'], geom=Point(lon, lat).wkt,
                                         daily_demand=total, match_quality=quality)
        ...
        self.notes = ['Typical autumn station entries + exits; weekday is Tuesday–Thursday. Not live footfall.',
