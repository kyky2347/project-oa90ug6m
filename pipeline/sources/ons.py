...
class ONSSource(Source):
    ...
    def discover(self):
        ...
        household = [
            u
            for _, u in links("https://www.nomisweb.co.uk/census/2021/bulk")
    ...
    def transform(self, directory, assets):
        ...
        total_col = next(
            (k for k in headers if "total" in k.lower() or k.startswith("Number of households:")), None
        )
        if not total_col:
        ...
        self.notes = [
            f"All-person population totals: mid-{year}. Households: Census 2021.",
