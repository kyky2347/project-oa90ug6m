...
def client():
    return httpx.Client(timeout=httpx.Timeout(120, connect=20), follow_redirects=True,
...
def links(url):
    ...
    return [(a.get_text(' ', strip=True), urljoin(str(r.url), a['href']))
    ...
    for attempt in range(4):
        try:
            with client() as c, c.stream('GET', asset.url) as r:
                ...
                return {'sha256': checksum, 'bytes': size, 'resolved_url': str(r.url),
...
class Source:
    ...
    def metadata(self):
        return {k: getattr(self, k) for k in ['id', 'name', 'publisher', 'url', 'license',
                                            'cadence_days', 'critical']}

    def register(self):
        execute('''INSERT INTO dataset_sources(id,name,publisher,url,license,cadence_days,critical)
            VALUES (:id,:name,:publisher,:url,:license,:cadence_days,:critical)
            ON CONFLICT(id) DO UPDATE SET name=excluded.name,url=excluded.url''', self.metadata())

    def previous(self):
        found = rows("SELECT * FROM dataset_snapshots WHERE source_id=:id AND status='verified' "
                     "ORDER BY retrieved_at DESC LIMIT 1", {'id': self.id})
    ...
    def load(self, tables, snapshot_id, conn):
        ...
        for table, records in tables.items():
            ...
            stmt = text(f"INSERT INTO {table}(snapshot_id,{','.join(keys)}) "
    ...
    def ingest(self, force=False):
        ...
        try:
            ...
            manifest = {'assets': downloaded, 'notes': getattr(self, 'notes', []),
                        'retrieved_at': datetime.now(UTC).isoformat()}
            checksum = hashlib.sha256(json.dumps([a['download']['sha256'] for a in downloaded]).encode()).hexdigest()
            meta = dict(id=sid, source=self.id, name=self.name, url=assets[0].url,
                        published=assets[0].published_at, license=self.license, checksum=checksum,
                        parser=self.parser_version, schema=self.schema_version,
                        path=str(directory.relative_to(ROOT)), metadata=json.dumps(manifest))
            with engine().begin() as conn:
                conn.execute(text('''INSERT INTO dataset_snapshots
                    ...
                    VALUES(:id,:source,:name,:url,:published,:license,:checksum,'staging',
                     :parser,:schema,:path,CAST(:metadata AS jsonb))'''), meta)
                count = self.load(tables, sid, conn)
                conn.execute(text("UPDATE dataset_snapshots SET status='verified',row_count=:n WHERE id=:id"),
                             {'n': count, 'id': sid})
            manifest.update(snapshot_id=sid, checksum=checksum, row_count=count, status='verified', **self.metadata())
            ...
            execute('''INSERT INTO dataset_snapshots
                ...
                dict(id=sid, source=self.id, name=self.name, url=assets[0].url if assets else self.url,
                     license=self.license, parser=self.parser_version, schema=self.schema_version,
                     path=str(directory.relative_to(ROOT)), error=str(exc)[:2000]))
            if previous:
                return previous['id']
            raise
