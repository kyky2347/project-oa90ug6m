...
BANDS = [('0_12000', 0, 12000), ('12001_15000', 12000, 15000),
         ('15001_50999', 15000, 51000), ('51000_499999', 51000, 500000),
...
class VOASource(Source):
    ...
    def discover(self):
        ...
        pages = sorted(set(pages), key=lambda u: max([int(y) for y in re.findall(r'20\d{2}', u)] or [0]), reverse=True)
        for page in pages:
    ...
    def transform(self, directory, assets):
        ...
        for borough in sorted(bands[0]):
            ...
            result.append(dict(code=borough, sector='all', property_count=sum(v or 0 for v in counts),
                               pressure=grouped_median(counts), method='borough_interpolated_median_rv_band'))
        if len(result) != 33:
            raise ValueError('VOA requires all 33 London boroughs')
        self.notes = ['Occupancy Cost Pressure Proxy: borough-level grouped median rateable value across commercial stock.',
