...
class GLASource(Source):
    ...
    def transform(self, directory, assets):
        ...
        col = next((k for k in df.columns if k.lower() in ('name', 'highstreet', 'hs_name', 'high_street_name', 'highstreet_name')), None)
        if not col:
        ...
        return {'geo_high_streets': [dict(id=str(i), name=str(r[col]), geom=valid_multi(r.geometry).wkt)
